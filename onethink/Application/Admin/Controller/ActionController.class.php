...
class ActionController extends AdminController {
    ...
    public function actionLog(){
        ...
        if(!empty($_REQUEST["nickname"])){
          $map['user_id']   =   array('eq', get_uid($_REQUEST["nickname"]));
           // $map = array('user_id'=>array('eq', get_uid($_REQUEST["nickname"])));
        }
        if (!empty($_REQUEST['firstDate']) ) {
            $map['create_time'][] = array('egt',strtotime(I('firstDate')));
        }
        if (!empty($_REQUEST['endDate']) ) {
            $map['create_time'][] = array('elt',24*60*60 + strtotime(I('endDate')));
        }
