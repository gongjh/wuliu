...
class ReceiveModel extends Model {
    protected $_validate = array(
        //array('receive_name', 'require', '收货人姓名不能为空', self::MUST_VALIDATE , 'regex', self::MODEL_INSERT),
        array('code', 'require', '快递单号不能为空', self::MUST_VALIDATE , 'regex', self::MODEL_BOTH),
        array('code', '', '快递单号已经存在', self::VALUE_VALIDATE, 'unique', 1),
       // array('city', 'require', '所在城市不能为空', self::MUST_VALIDATE , 'regex', self::MODEL_BOTH),
       // array('receive_phone', 'require', '电话不能为空', self::MUST_VALIDATE , 'regex', self::MODEL_BOTH),
    );
