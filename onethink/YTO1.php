...
$fp = fopen('D:/YTO1_log.txt','a+');
//$fp = fopen('/home/wwwroot/default/onethink/yto1_log',"a+");
fwrite($fp, "-----------------------------------------------");
fwrite($fp, $info);
fwrite($fp, "----------------------------------------------- \r\n");
$result = @mysql_query("select * from wuliu_sendonline where type=1 and company_id =3 and FROM_UNIXTIME(receive_time,'%Y-%m-%d')=curdate() and receive_time+6*3600<=unix_timestamp()");
while(!!$row = mysql_fetch_array($result)){
    @mysql_query("UPDATE wuliu_sendonline SET type=4 WHERE id=".$row["id"]);
