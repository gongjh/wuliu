...
$fp = fopen('D:/other_log.txt','a+');
//$fp = fopen('/home/wwwroot/default/onethink/other_log',"a+");
fwrite($fp, "-----------------------------------------------");
fwrite($fp, $info);
fwrite($fp, "----------------------------------------------- \r\n");
$result = mysql_query("select * from wuliu_sendonline  where  (type=2 and company_id =3) or (company_id!=3 and type!=4) and FROM_UNIXTIME(receive_time,'%Y-%m-%d')=curdate() ");
//print_r($result);
