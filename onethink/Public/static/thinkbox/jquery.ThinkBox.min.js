...
(function($){
	...
	var wrapper = [	'<div class="ThinkBox-wrapper">', //弹出框外层容器
						'<table cellspacing="0" cellpadding="0" border="0">', //使用表格，可以做到良好的宽高自适应，而且方便做圆角样式
							...
							'</tr>',
							'<tr>',
								'<td class="box-left"><div></div></td>',       //左边
								'<td class="ThinkBox-inner"></td>', //弹出框inner
								'<td class="box-right"><div></div></td>',      //右边
							'</tr>',
							'<tr>',
	...
	var ThinkBox = function(element, options){
		...
		function _show() {
			...
			function _(){
				...
				!options.is_load && _fire.call(self, options.afterShow);
			}
		};
		...
		function _setupModal(){
			...
			$(window).resize(function() {
				modal&&modal.css({'width' : "", 'height' : ""});
				modal&&modal.css({'width' : $(document).width(), 'height' : $(document).height()});
			});
		}
	...
	$.extend($.ThinkBox, {
		...
		load : function(url, opt){
			var options = {'type' : 'GET', 'dataType' : 'text', 'cache' : false, 'parseData':undefined, 'is_load':true};
			$.extend(options, opt || {});
		...
		'success' : function(msg, opt){
			var options = {'style' : 'style01'};
			$.extend(options, opt || {});
			return this.tips(msg, 'success', options);
		},
		...
		'error' : function(msg, opt){
			var options = {'style' : 'style01'};
			$.extend(options, opt || {});
			return this.tips(msg, 'error', options);
		},
		...
		return this.each(function(){
			...
			switch(opt){
				...
				default:
					...
					$.extend(options, {
						'x' : function(){
							var size = $(this).ThinkBox('get').getSize();
							return Math.min($(this).offset().left, $(window).width() - size[0]);
						}, 
